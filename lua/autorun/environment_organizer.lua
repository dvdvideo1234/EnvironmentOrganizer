...
local envOrg = "EnvironmentOrganizer: "
local envGetCV = "envGetConvarValue: "
...
if ( envEn ) then
		...
		if ( envEn ) then
			for ID = 1, #tMembers do
				...
				if ( envMember[3] ~= nil ) then
					...
					print( tMembers.NAM.."."..envMember[3], tMembers.OLD[envMember[2]], tMembers.NEW[envMember[2]] )
				else -- Scalar value
					tMembers.NEW = envGetConvarValue( envMember[1] )
					print( tMembers.NAM, tMembers.OLD, tMembers.NEW )
				end
			end
		else
			print( envOrg..tMembers.NAM..": Extension disabled" )
		end
	end

	local function envDumpConvars( tMembers )
		local Out = ( tMembers.NAM.."\n" )
		for ID = 1, #prefMembers do
	...
	local function envDumpStatus( tMembers, sStatus )
		...
		if ( Key ~= "NEW" ) or ( Key ~= "OLD" ) then return end
  ...
  -- https://wiki.garrysmod.com/page/Category:physenv
	...
	local function envPrintDelta( anyParam, anyOld, anyNew )
		print( envOrg.."["..tostring( anyParam ).."] Old<"..tostring( anyOld ).."> New<"..tostring( anyNew )..">")
	end
